...
CONTAINER_BASE_PATH = "{{ mi_os.path.dirname(dag.fileloc) }}/../.." # #  usamos 'mi_os' que existe en el contexto, fue importado en la definicion del dag
...
def extract_from_api() -> str:
    ...
    filename = f"raw_customers_{int(datetime.now(timezone.utc).timestamp())}.csv"
...
@dag(
    ...
    # Inyectamos el módulo os para que esté disponible en Jinja como 'mi_os'
    user_defined_macros={'mi_os': os}
)
def complex_data_pipeline_beam():
    ...
    normalized_filename = BeamRunPythonPipelineOperator(
        ...
        runner="DirectRunner",
        #py_requirements=["apache-beam[gcp]==2.73.0"],
    )
